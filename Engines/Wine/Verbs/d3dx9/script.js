...
Wine.prototype.d3dx9 = function () {
    const extractDirectXtoSystemDirectory = (progressBar, filesToExtract, destination, pattern) => {
        filesToExtract.reduce((numberOfExtractedFiles, cabFile) => {
            ...
            new CabExtract()
                .archive(this.prefixDirectory() + "/drive_c/d3dx9/" + cabFile)
                .to(destination)
                .extract(["-L", "-F", pattern]);
    ...
    extractDirectXtoSystemDirectory(progressBar, filesToExtractx86, this.system32directory(), "d3dx9*.dll");
    ...
    if (this.architecture() == "amd64") {
        ...
        extractDirectXtoSystemDirectory(progressBar, filesToExtractx64, this.system64directory(), "d3dx9*.dll");
    }
...
module.default = class D3DX9Verb {
    ...
    install(container) {
        ...
        const wizard = SetupWizard(InstallationType.VERBS, "d3dx9", java.util.Optional.empty());
