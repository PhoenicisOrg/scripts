...
const { cat, fileExists, writeToFile, createTempFile } = include(
  "utils.functions.filesystem.files"
);
...
module.default = class SteamScript extends QuickScript {
  ...
  fixCertificateIssue(wine) {
    ...
    const newSteamConfig =
      steamConfig.slice(0, cmPos) + csConfig + steamConfig.slice(cmPos);
  ...
  go() {
    ...
    wine.runInsidePrefix(
      `${wine.programFiles()}/Steam/steam.exe`,
      ["steam://nav/games"],
      false
    );
    ...
    wine.runInsidePrefix(
      `${wine.programFiles()}/Steam/steam.exe`,
      [`steam://install/${this._appId}`],
      false
    );

    setupWizard.wait(
      tr("Please wait until Steam has finished the download...")
    );
